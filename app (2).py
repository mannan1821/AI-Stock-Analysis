...
def _build_chart(symbol: str, period: str, compare_symbol: Optional[str], indicators: List[str]):
    ...
    tickers. Returns (figure, summary_text) or (None, error_text)."""
...
def show_stock_chart(
    ...
    fig, summary = _build_chart(symbol, period, compare_symbol, indicators)
    if fig is not None:
        st.session_state["_pending_chart"] = fig
    return summary
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("Thinking..."):
            ...
            if chart is not None:
                st.plotly_chart(chart, use_container_width=True, key="chart_new")
            if summary:
                with st.expander("📋 What I looked up"):
                    st.markdown(summary)
